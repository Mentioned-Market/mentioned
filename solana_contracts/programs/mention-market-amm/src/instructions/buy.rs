...
pub fn handle_buy(
    ...
    require!(escrow.balance >= total_cost, AmmError::InsufficientBalance);
    escrow.balance = escrow.balance.checked_sub(total_cost).ok_or(AmmError::MathOverflow)?;

    // Transfer lamports from escrow PDA to vault
    **ctx.accounts.trader_escrow.to_account_info().try_borrow_mut_lamports()? -= total_cost;
    **ctx.accounts.vault.to_account_info().try_borrow_mut_lamports()? += total_cost;
    ...
    match direction {
        Side::Yes => {
            word.yes_quantity = word.yes_quantity.checked_add(quantity as i64).ok_or(AmmError::MathOverflow)?;
        }
        Side::No => {
            word.no_quantity = word.no_quantity.checked_add(quantity as i64).ok_or(AmmError::MathOverflow)?;
        }
    }
    ...
    msg!(
        "Buy {} {:?} tokens for word {} — cost: {} lamports (fee: {})",
        quantity,
        direction,
        word_index,
        total_cost,

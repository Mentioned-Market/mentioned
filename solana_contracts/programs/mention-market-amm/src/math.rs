...
///
/// All values are scaled by PRECISION = 1e9.
/// Example: 0.65 SOL is stored as 650_000_000.
...
/// ln(2) scaled by 1e9 ≈ 693_147_180
const LN2: i128 = 693_147_180;

// ---------------------------------------------------------------------------
// Fixed-point exp(x) where x is scaled by 1e9
//
...
// Returns u128 scaled by 1e9.
// ---------------------------------------------------------------------------

/// Fixed-point exp. Input: i64 scaled by 1e9. Output: u128 scaled by 1e9.
...
// ---------------------------------------------------------------------------
// Fixed-point ln(x) where x is u128 scaled by 1e9
//
// Strategy: decompose x = m * 2^k where 1 <= m < 2 (in fixed-point).
//   ln(x) = k * ln(2) + ln(m)
// ln(m) is approximated with a polynomial for m in [1, 2).
//
// Returns i64 scaled by 1e9.
// ---------------------------------------------------------------------------

/// Fixed-point natural log. Input: u128 scaled by 1e9 (must be > 0). Output: i64 scaled by 1e9.
...
///
/// All inputs/outputs scaled by PRECISION (1e9).
...
/// Returns the cost in lamports (scaled by 1e9).
...
/// Cost recovered when selling `amount` of YES or NO tokens.
/// Returns the amount in lamports (scaled by 1e9).

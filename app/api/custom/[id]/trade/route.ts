...
import { getCustomMarket, getCustomMarketWords, executeVirtualTrade, lockCustomMarket, getRecentCustomTradeCount, assertDiscordTradingEligible, countWalletDistinctMarketsThisWeek } from '@/lib/db'
...
export async function POST(
  ...
  try {
    ...
    try {
      const distinctCount = await countWalletDistinctMarketsThisWeek(wallet)
      if (distinctCount >= 3) {
        const ach = await tryUnlockAchievement(wallet, 'trade_3_markets')
        if (ach) newAchievements.push({ id: ach.id, emoji: ach.emoji, title: ach.title, points: ach.points })
      }
    } catch (err) {
      console.error('Achievement error (trade_3_markets):', err)
    }
